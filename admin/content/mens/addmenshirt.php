...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($product_id) {
        ...
        $stmt = $conn->prepare("UPDATE men_shirts_products SET product_name = ?, price = ?, image = ? WHERE id = ?");
        $stmt->bind_param("sssi", $product_name, $price, $imagePath, $product_id);
    } else {
     
        $stmt = $conn->prepare("INSERT INTO men_shirts_products (product_name, price, image) VALUES (?, ?, ?)");
    ...
    if ($stmt->execute()) {
        ...
        if (!empty($galleryImages)) {
            foreach ($galleryImages as $imgName) {
                $stmt = $conn->prepare("INSERT INTO men_shirts_photos (product_id, photo_path) VALUES (?, ?)");
        ...
        echo "<script>window.location.href = '?page=mens/viewmen';</script>";
    } else {
      ...
      <section class="content-header">
        <div class="container-fluid">
          <div class="row mb-2">
            ...
            <div class="col-sm-6">
              <a href="?page=mens/viewmen" class="btn btn-success"><i class="fa fa-eye" aria-hidden="true"></i> Виж Продукти</a>
            </div>
          </div>
        </div>
      </section>
