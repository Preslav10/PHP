...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($product_id) {
        ...
        $stmt = $conn->prepare("UPDATE women_pants_products SET product_name = ?, price = ?, image = ? WHERE id = ?");
        $stmt->bind_param("sssi", $product_name, $price, $imagePath, $product_id);
    } else {
        
        $stmt = $conn->prepare("INSERT INTO women_pants_products (product_name, price, image) VALUES (?, ?, ?)");
    ...
    if ($stmt->execute()) {
        ...
        if (!empty($galleryImages)) {
            foreach ($galleryImages as $imgName) {
                $stmt = $conn->prepare("INSERT INTO women_pants_photos (product_id, photo_path) VALUES (?, ?)");
        ...
        echo "<script>window.location.href = '?page=womens/viewpants';</script>";
    } else {
