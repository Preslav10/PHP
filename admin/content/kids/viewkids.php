...
function filterProducts() {
    ...
    fetch(`fetch_products_kids.php?category=${selectedCategory}`)
        ...
        .then(data => {
            ...
            if (data.length > 0) {
                data.forEach(product => {
                    ...
                    row.innerHTML = `
                    <td><img src="content/kids/images/albums/${product.image}" alt="Снимка" style="max-width: 100px; max-height: 100px;"></td>
                    <td>${product.product_name}</td>
                    <td>${product.price} лв.</td>
                    <td>
                        <a href="edit3.php?id=${product.id}&category=${selectedCategory}" class='btn btn-sm btn-warning'>Редактирай</a>
                        <form method='POST' action='./handlers/delate/delate_kids.php' style='display:inline;'>
