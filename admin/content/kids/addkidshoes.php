...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if ($product_id) {
        ...
        $stmt = $conn->prepare("UPDATE kids_shoes_products SET product_name = ?, price = ?, image = ? WHERE id = ?");
        $stmt->bind_param("sssi", $product_name, $price, $imagePath, $product_id);
    } else {
       
        $stmt = $conn->prepare("INSERT INTO kids_shoes_products (product_name, price, image) VALUES (?, ?, ?)");
    ...
    if ($stmt->execute()) {
        ...
        if (!empty($galleryImages)) {
            foreach ($galleryImages as $imgName) {
                $stmt = $conn->prepare("INSERT INTO kids_shoes_photos (product_id, photo_path) VALUES (?, ?)");
